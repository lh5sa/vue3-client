<template>
  <el-dialog title="修改权限信息" :model-value="updateLayerVisible" @closed="hideLayer">
...
import { inject, watch, reactive, readonly, ref, computed } from "vue";
/* prettier-ignore */
const {
  updateLayerVisible,
...
const submitForm = () => {
  formRef.value.validate(async (isPass) => {
    ...
    await updatePerimInfo(formData);
    hideLayer();
  });
};
</script>
